...
export default class JurisdictionHistoryRow extends Component<Props> {
  ...
  render() {
    ...
    if (currentSequence) {
      if (jurisdictions.ALL_FOR_BLOCK === 'FAILURE') {
        this.rightError = true;
      } else if (jurisdictions.ALL_FOR_BLOCK === 'SUCCESS') {
        ...
        <Grid.Row>
          <Grid.Column>
            ...
            <div className="history-scroll">
              ...
              {(currentSequence ? jurisdictions.ALL_FOR_TRANSACTION === 'FAILURE' : this.leftError) &&
          ...
          <Grid.Column>
            ...
            <div className="history-scroll">
              {rightRows.length > 0 && (currentSequence ? jurisdictions.ALL_FOR_BLOCK === 'SUCCESS' : !this.rightError) && rightRows.map((row, idx) => (
                <p key={idx} className="history-wrapper">{`${row.name} (${row.description})`}</p>
              ))}
              {rightRows.length === 0 && (currentSequence ? jurisdictions.ALL_FOR_BLOCK === 'SUCCESS' : !this.rightError) &&
                <p>No jurisdictions.</p>
              }
              {(currentSequence ? jurisdictions.ALL_FOR_BLOCK === 'PENDING' : false) &&
                <p>Loading...</p>
              }
              {(currentSequence ? jurisdictions.ALL_FOR_BLOCK === 'FAILURE' : this.rightError) &&
                <p>Error fetching data.</p>
              }
            </div>
          </Grid.Column>
